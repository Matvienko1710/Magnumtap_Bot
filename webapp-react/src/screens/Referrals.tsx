...
export default function Referrals({user}:Props){
	...
	const bot=(window as any).Telegram?.WebApp?.initDataUnsafe?.start_param||'';
	const link= uid? `https://t.me/${(window as any).Telegram?.WebApp?.bot?.username||'your_bot'}?start=${uid}` : '';
	async function copy(){
		try{
