...
async function showRoadmapQ2_2025(ctx, user) {
  try {
    ...
    const message = 
      ...
      `├ Рулетка с различными ставками\n` +
      `├ Карточные игры\n` +
      `└ Лотереи с джекпотами\n\n` +
...
async function showRoadmapVote(ctx, user) {
  try {
    const keyboard = Markup.inlineKeyboard([
      ...
      [
        Markup.button.callback('🎰 Казино', 'vote_casino'),
        Markup.button.callback('🤖 AI', 'vote_ai')
      ],
      [
    ...
    const message = 
      ...
      `🎰 *Казино и азартные игры* - 28% (654 голоса)\n` +
      `🤖 *AI помощник* - 25% (543 голоса)\n` +
...
async function showRoadmapSuggestions(ctx, user) {
  try {
    ...
    const message = 
      ...
      `├ Укажите тему: "Предложение для роадмапа"\n` +
      `├ Опишите вашу идею подробно\n` +
      `└ Укажите желаемый срок реализации\n\n` +
      `🎯 *Лучшие предложения получат награды:*\n` +
      `├ 🥇 1 место: 10,000 Magnum Coins\n` +
      `├ 🥈 2 место: 5,000 Magnum Coins\n` +
      `└ 🥉 3 место: 2,500 Magnum Coins\n\n` +
      `📅 *Следующий конкурс идей: 1 февраля 2025*\n\n` +
