...
async function showExchangeMenu(ctx, user) {
  const keyboard = Markup.inlineKeyboard([
    [
      Markup.button.callback('⭐ Stars → 🪙 Magnum Coins', 'exchange_stars_to_coins'),
      Markup.button.callback('🪙 Magnum Coins → ⭐ Stars', 'exchange_coins_to_stars')
    ],
    [
      Markup.button.callback('📊 Курсы обмена', 'exchange_rates'),
      Markup.button.callback('📈 История', 'exchange_history')
    ],
    [Markup.button.callback('🔙 Назад', 'main_menu')]
  ]);
...
async function showWithdrawalMenu(ctx, user) {
  const keyboard = Markup.inlineKeyboard([
    [
      Markup.button.callback('💰 Вывести Stars', 'withdraw_stars'),
      Markup.button.callback('💳 Вывести Magnum Coins', 'withdraw_coins')
    ],
    [
  ...
  const message = 
    ...
    `└ 🪙 Magnum Coins: \`${formatNumber(user.magnumCoins)}\`\n\n` +
    `📋 *Условия:*\n` +
    `├ Минимум: ${config.MIN_WITHDRAWAL}\n` +
    `├ Максимум: ${config.MAX_WITHDRAWAL}\n` +
    `└ Заявок: ${user.withdrawal.withdrawalCount}\n\n` +
    `🎯 Выберите действие:`;
...
async function getBotStatistics() {
  try {
    const cached = getCachedStats('bot_stats');
    if (cached) return cached;
    ...
    const stats = await db.collection('users').aggregate([
      {
        $group: {
          ...
          totalReferrals: { $sum: { $ifNull: ['$referralsCount', 0] } },
          totalAchievements: { $sum: { $ifNull: ['$achievementsCount', 0] } }
        }
      }
    ]).toArray();
    ...
    setCachedStats('bot_stats', result);
    return result;
  } catch (error) {
...
bot.action('exchange', async (ctx) => {
  try {
    ...
    console.error('Ошибка меню обмена:', error);
  }
});
...
bot.action('withdrawal', async (ctx) => {
  try {
    ...
    console.error('Ошибка меню вывода:', error);
  }
});
